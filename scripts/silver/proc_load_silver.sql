...
Parameters:
  None.
  This storedprocedure does not accept any parameters or return any values.
		...
		PRINT '>> Inserting Data Into: silver.crm_cust_info';
			...
			SELECT
				...
				CASE
					...
					ELSE 'n/a'
				END AS cst_marital_status, -- Data Normalisation & Standardisation: Normalise marital status valued to readable format
				CASE
					WHEN TRIM(UPPER(cst_gender)) = 'F' THEN 'Female'
					WHEN TRIM(UPPER(cst_gender)) = 'M' THEN 'Male'
					ELSE 'n/a'
				END AS cst_gender, -- Data Normalisation & Standardisation: Normalise gender values to readable format
				cst_create_date
			FROM (
				SELECT
				*,
				...
				) AS t WHERE flag_last = 1 -- Removing duplicates and selecting most recent record per customer
				;
		...
		PRINT '>> Inserting Data Into: silver.crm_sales_details';
			...
			SELECT
				...
				CASE
					...
					ELSE CAST(CAST(sls_order_dt AS VARCHAR) AS DATE) -- Cast as VARCHAR before casting to DATE because cannot convert an INT directly to DATE in SQL Server
				END AS sls_order_dt,
				CASE
